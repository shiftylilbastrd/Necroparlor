...
1. **Plug in the webcam and find its device index:**
   ...
   The Config page's Camera card has its own **Discover** button that does the exact same probing without SSHing in - it shows each working index as a small thumbnail (so you can tell which one is actually pointed into the enclosure) with its resolution. Click one to select it (it highlights, matching the External card's Discover table) and fill in both the device field *and* the width/height fields below - the probe explicitly requests 1920x1080 before reading each device back, so what shows is the camera's actual negotiated capability (V4L2 will report back whichever resolution it actually supports closest to that), not OpenCV's out-of-the-box default of 640x480 that you'd get without asking for anything. That's a starting point, not something you're locked into - lower it from there for a CPU/bandwidth tradeoff on a Pi 3B+, same as the live-view interval note above. Nothing is saved to disk this way (the sample frame comes back inline as the button's own result, not a file next to the script), so repeated clicks don't leave old `discover_camera_N.jpg` files scattered around the project directory. It also handles the one thing the CLI version leaves to you: if `dermestid-camera.service` is already running, it's holding the real camera device open (most webcams only allow one client at a time), so the button briefly stops that service, probes, then starts it again - this needs the two extra `sudoers` lines from the "one-click apply" section above (`stop`/`start dermestid-camera.service`); without them Discover still runs, it just won't see whichever index the service already has open.
